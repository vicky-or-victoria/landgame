...
from utils.checks import is_owner, setup_complete
...
class TerritoryView(discord.ui.View):
    ...
    @discord.ui.button(label="Claim Tile", style=discord.ButtonStyle.secondary, custom_id="territory:claim")
    async def claim(self, interaction, button):
        await interaction.response.send_modal(ClaimTileModal())

    @discord.ui.button(label="Develop Tile", style=discord.ButtonStyle.secondary, custom_id="territory:develop")
...
class MilitaryView(discord.ui.View):
    ...
    async def view_armies(self, interaction, button):
        ...
        armies = await get_armies(interaction.client, interaction.user.id)
        if not armies:
...
class EconomyView(discord.ui.View):
    ...
    async def balance(self, interaction, button):
        ...
        p = await get_player(interaction.client, interaction.user.id)
        if not p:
    ...
    async def tax(self, interaction, button):
        ...
        result = await collect_tax(interaction.client, interaction.user.id)
...
class PoliticsView(discord.ui.View):
    ...
    async def traditions(self, interaction, button):
        ...
        t = await get_traditions(interaction.client, interaction.user.id)
...
class DiplomacyView(discord.ui.View):
    ...
    async def view_treaties(self, interaction, button):
        ...
        treaties = await get_treaties(interaction.client, interaction.user.id)
        if not treaties:
...
class InfoView(discord.ui.View):
    ...
    async def view_map(self, interaction, button):
        ...
        img = await render_map(interaction.client)
        await interaction.followup.send(file=img, ephemeral=True)

    @discord.ui.button(label="Inspect Tile", style=discord.ButtonStyle.secondary, custom_id="info:inspect_tile")
    ...
    async def leaderboard(self, interaction, button):
        ...
        rows = await get_leaderboard(interaction.client)
...
def main_menu_embed():
    ...
    e.description = (
        ...
        "Territory  — claim, develop, and build on tiles\n"
...
class MainMenuView(discord.ui.View):
    ...
    async def territory(self, interaction, button):
        ...
        e.description = "Claim, develop, and construct buildings on your tiles."
    ...
    async def info(self, interaction, button):
        e = embeds.info("Info")
        e.description = "View the map, inspect tiles, and check the leaderboard."
...
class Menu(commands.Cog):
    ...
    async def deploy_menu(self, interaction: discord.Interaction):
        cfg = self.bot.config
        channel_id = cfg.get_channel("menu")
        if not channel_id:
