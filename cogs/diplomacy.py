...
async def handle_offer_treaty(interaction: discord.Interaction, target_name: str, treaty_type: str):
    ...
    if target["discord_id"] == interaction.user.id:
        ...
        return
    await offer_treaty(bot, guild_id, interaction.user.id, target["discord_id"], treaty_type)
    ...
    await post_public(bot, guild_id, mention, embeds.info(f"Treaty Offer — {treaty_type.title()}", f"{interaction.user.display_name} offered a {treaty_type} to {target_name}."))
...
async def handle_declare_war(interaction: discord.Interaction, target_name: str):
    ...
    if target["discord_id"] == interaction.user.id:
        ...
        return
    if await is_in_grace(bot, guild_id, target["discord_id"]):
        ...
        return
    declared = await declare_war(bot, guild_id, interaction.user.id, target["discord_id"])
    if not declared:
    ...
    target_user = bot.get_user(target["discord_id"])
    mention = target_user.mention if target_user else target_name
    await post_public(bot, guild_id, mention, embeds.battle(f"War Declared", f"{interaction.user.display_name} has declared war on {target_name}. Hostilities begin in 24 hours."))
