...
async def handle_market_order(interaction: discord.Interaction, resource: str, amount: int, price: int, order_type: str):
    ...
    if order_type not in ("buy", "sell"):
        ...
        return
    await post_market_order(bot, interaction.user.id, resource, amount, price, order_type)
    ...
    await post_public(bot, interaction.user.mention, embeds.info(f"Market Order — {resource.title()}", f"{interaction.user.display_name} posted: {order_type} {amount} {resource} @ {price} each."))
...
async def handle_trade(interaction: discord.Interaction, target_name: str, resource: str, amount: int):
    ...
    if not target:
        ...
        return
    success = await transfer_resources(bot, interaction.user.id, target["discord_id"], resource, amount)
    if not success:
    ...
    await post_public(bot, mention, embeds.success("Trade", f"{interaction.user.display_name} sent {amount} {resource} to {target_name}."))
