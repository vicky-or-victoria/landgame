...
async def post_public(bot, content: str, embed: discord.Embed):
...
async def handle_claim(interaction: discord.Interaction, coord: str):
    ...
    tile = await tile_queries.get_tile(bot, coord)
    if not tile:
    ...
    if tile["owner_id"]:
        ...
        return
    player = await player_queries.get_player(bot, interaction.user.id)
    if not player:
    ...
    if player["gold"] < cost:
        ...
        return
    await tile_queries.set_owner(bot, coord, interaction.user.id)
    await player_queries.adjust_resources(bot, interaction.user.id, gold=-cost)
    await interaction.followup.send(embed=embeds.success("Tile Claimed", f"You claimed {coord} for {cost} gold."), ephemeral=True)
    await post_public(bot, interaction.user.mention, embeds.success(f"Tile Claimed — {coord}", f"{interaction.user.display_name} claimed tile {coord}."))
...
async def handle_develop(interaction: discord.Interaction, coord: str, amount: int):
    ...
    if player["gold"] < amount:
        ...
        return
    dev_gain = amount // 20
    await tile_queries.adjust_dev(bot, coord, dev_gain)
    await player_queries.adjust_resources(bot, interaction.user.id, gold=-amount)
    await interaction.followup.send(embed=embeds.success("Tile Developed", f"{coord} gained +{dev_gain} dev."), ephemeral=True)
    await post_public(bot, interaction.user.mention, embeds.success(f"Development — {coord}", f"{interaction.user.display_name} invested {amount} gold into {coord}."))
...
async def handle_build(interaction: discord.Interaction, coord: str, building_name: str):
    ...
    buildings = await get_buildings(bot, coord)
    ...
    if len(buildings) >= max_slots:
        ...
        return
    await add_building(bot, coord, building_name)
    await interaction.followup.send(embed=embeds.success("Building Constructed", f"{building_name} built on {coord}."), ephemeral=True)
    await post_public(bot, interaction.user.mention, embeds.success(f"Construction — {coord}", f"{interaction.user.display_name} built {building_name} on {coord}."))
...
async def handle_demolish(interaction: discord.Interaction, coord: str, building_name: str):
    ...
    removed = await remove_building(bot, coord, building_name)
    if not removed:
    ...
    await post_public(bot, interaction.user.mention, embeds.success(f"Demolition — {coord}", f"{interaction.user.display_name} demolished {building_name} on {coord}."))
