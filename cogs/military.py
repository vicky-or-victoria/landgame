...
async def handle_raise_levy(interaction: discord.Interaction, coord: str):
    ...
    if levy_size < 1:
        ...
        return
    army_id = await raise_levy(bot, interaction.user.id, coord, levy_size)
    ...
    await post_public(bot, interaction.user.mention, embeds.success(f"Levy Raised — {coord}", f"{interaction.user.display_name} raised a levy of {levy_size} at {coord}."))
...
async def handle_move_army(interaction: discord.Interaction, army_id: int, coord: str):
    ...
    if not tile:
        ...
        return
    moved = await move_army(bot, army_id, interaction.user.id, coord)
    if not moved:
    ...
    await post_public(bot, interaction.user.mention, embeds.success(f"Army Moved — {coord}", f"{interaction.user.display_name} moved Army #{army_id} to {coord}."))

async def handle_assign_front(interaction: discord.Interaction, army_id: int, coord: str):
    await interaction.response.defer(ephemeral=True)
    bot = interaction.client
    assigned = await assign_to_front(bot, army_id, interaction.user.id, coord)
    if not assigned:
    ...
    await post_public(bot, interaction.user.mention, embeds.battle(f"Front Established — {coord}", f"{interaction.user.display_name} assigned Army #{army_id} to the front at {coord}."))
