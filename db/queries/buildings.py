...
async def get_buildings(bot, coord: str):
    pool = await get_pool()
    rows = await pool.fetch("SELECT * FROM buildings WHERE tile_coord = $1", coord)
    return [dict(r) for r in rows]

async def add_building(bot, coord: str, name: str):
    pool = await get_pool()
    name = name.lower()
    defn = BUILDING_DEFINITIONS.get(name, {"tier": 1, "category": "misc"})
    await pool.execute(
        "INSERT INTO buildings (tile_coord, name, category, tier) VALUES ($1, $2, $3, $4)",
        coord, name.title(), defn["category"], defn["tier"]
    )

async def remove_building(bot, coord: str, name: str) -> bool:
    pool = await get_pool()
    result = await pool.execute(
        "DELETE FROM buildings WHERE tile_coord = $1 AND LOWER(name) = $2",
        coord, name.lower()
    )
